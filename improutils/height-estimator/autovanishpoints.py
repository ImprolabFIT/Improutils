...
# Algorithm for automatic calcuation of vanishing points/lines in image
...
def compute_edgelets(image, sigma=3):
    ...
    """
    gray_img = cv2.color.rgb2gray(image)
    edges = cv2.feature.canny(gray_img, sigma)
    lines = cv2.transform.probabilistic_hough_line(edges, line_length=3,
                                               line_gap=2)
    locations = []
    directions = []
    strengths = []
...
def compute_homography_and_warp(image, vp1, vp2, clip=True, clip_factor=3):
    ...
    warped_img = cv2.transform.warp(image, np.linalg.inv(final_homography),
